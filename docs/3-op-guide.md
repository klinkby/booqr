...
> Operational runbook for the shared-schema + RLS + per-tenant-role model. Design rationale is in
> `docs/1-design.md`; build detail in `docs/implementation.md`. This guide assumes that design is
> implemented — the `admin` CLI commands referenced here are defined in `docs/1-design.md` §2b.
...
- **Provision:** `docker compose run --rm admin --provision <slug>`
  - `<slug>` must be a DNS label (`^[a-z0-9]([a-z0-9-]{0,30}[a-z0-9])?$`). Reserved: `www`, `api`.
